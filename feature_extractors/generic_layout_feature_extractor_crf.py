...
class FeatureExtractor(object):
    ...
    def get_features(self, text, key_prefix='', is_not_padding=True):
        ...
        features = {key_prefix + '_all_lower': str(clean_text.islower()) if is_not_padding else -1,
                    ...
                    key_prefix + '_contains_special_char_inside': self.contains_special_char_inside(
                        text) if is_not_padding else -1,
                    key_prefix + '_heading_startswith_isda_para': self.heading_startswith_isda_para(
                        text) if is_not_padding else -1,
                    key_prefix + '_heading_startswith_isda_part': self.heading_startswith_isda_part(
                        text) if is_not_padding else -1
                    }
        return features
    ...
    def heading_startswith_isda_para(self, text):
        regex0 = r'Paragraph\s{0,4}\d{1,3}\.?\s{0,4}\w{4,50}'
        regex1 = r'Paragraph\s{0,4}\d{1,3}\.?\s{0,4}'
        if re.search(regex0, text) or re.search(regex1, text):
            return 1
        else:
            return 0

    def heading_startswith_isda_part(self, text):
        regex0 = r'^Part\s{1,3}\d{1,2}\.?'
        regex1 = r'^PART\s{1,3}\d{1,2}\.?'
        if re.search(regex0, text) or re.search(regex1, text):
            return 1
        else:
            return 0
