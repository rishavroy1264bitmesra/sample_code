...
class LineIdsResponseJSONGenerator(AbstractJSONGenerator):
    ...
    def _get_all_paras(self, paraQueue, dummyRowContent):
        if len(paraQueue) == 0:
            ...
            while (len(paraQueue) > 0): para.merge_subsections_and_lineids(paraQueue.popleft())
        return para
    ...
    def _get_clause(self, heading, subheading, para):
        ...
        clause['Main-heading'] = {str(heading.para_id): heading.buffer}
        clause['Main_heading_No'] = self.bullet_detector.get_bullet(heading.buffer)
        clause['Sub-heading'] = {str(subheading.para_id): subheading.buffer}
    ...
    def split_para(self, para):
        ...
        if len(sub_sections_texts_array) == len(sub_sections_lineids):
            for index in range(0, len(sub_sections_texts_array)):
                if len(sub_sections_texts_array[index]) > 0:
                    result.append({
                        str(sub_sections_lineids[index]): sub_sections_texts_array[index]
                    })
        else:
